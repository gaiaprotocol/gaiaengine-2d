...
export default class TouchChecker extends TreeNode {
  ...
  constructor(
    ...
    private eventHandler: (x: number, y: number) => void,
  ) {
    super();
  ...
  private domEventHandler = (e: TouchEvent | MouseEvent) => {
    if (this._screen && this.parent) {
      ...
      const rx = ((e instanceof TouchEvent ? e.touches[0].clientX : e.clientX) -
            screenRect.x - this._screen.width / 2 + this._screen.camera.x) /
      ...
      const ry = ((e instanceof TouchEvent ? e.touches[0].clientY : e.clientY) -
            screenRect.y - this._screen.height / 2 + this._screen.camera.y) /
