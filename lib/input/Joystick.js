...
export default class Joystick extends GameObject {
    ...
    handleTouchStart = (event) => {
        ...
        if (this.options.joystickImage) {
            this.joystickSprite = new Sprite(this.touchStartX, this.touchStartY, this.options.joystickImage);
        }
        if (this.options.knobImage) {
            this.knobSprite = new Sprite(this.touchStartX, this.touchStartY, this.options.knobImage);
        }
    };
    handleTouchMove = (event) => {
        ...
        for (let i = 0; i < event.changedTouches.length; i++) {
            ...
            if (touch.identifier === this.activeTouchId) {
                ...
                if (this.knobSprite) {
                    this.knobSprite.setPosition(this.touchStartX + clampedX, this.touchStartY + clampedY);
                }
