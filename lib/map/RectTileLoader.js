...
export default class RectTileLoader extends GameObject {
    ...
    loadTiles() {
        if (this.screen) {
            ...
            const endTileX = Math.ceil((boundRight + this.tileSize / 2) / this.tileSize) + extraLoadTileCount;
            const startTileY = Math.floor((boundTop + this.tileSize / 2) / this.tileSize) - extraLoadTileCount;
            const endTileY = Math.ceil((boundBottom + this.tileSize / 2) / this.tileSize) + extraLoadTileCount;
