...
export default class RectMap extends RectTileLoader {
    ...
    async loadSpritesheets() {
        await Promise.all(Object.entries(this.spritesheets).map(([id, src]) => SpritesheetLoader.load(id, src, this.atlases[id])));
    ...
    renderTerrain(x, y, terrainId, direction) {
        ...
        if (entry.frames.length > 1) {
            ...
            tile = new RectMapTerrain(x * this.tileSize, y * this.tileSize, spritesheetSrc, this.atlases[entry.spritesheet], `frame_${frame.x}_${frame.y}_${frame.width}_${frame.height}`, this._options.tileFadeDuration);
        }
        if (tile) {
            ...
            if (objectX === x && objectY === y) {
                ...
                if (objectInfo) {
                    ...
                    if (objectInfo.frames.length > 1) {
                        ...
                        object = new RectMapObject(mapObject.x, mapObject.y, spritesheetSrc, this.atlases[objectInfo.spritesheet], `frame_${frame.x}_${frame.y}_${frame.width}_${frame.height}`, this._options.tileFadeDuration);
                    }
